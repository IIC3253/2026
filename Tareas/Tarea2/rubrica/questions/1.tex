...
La corrección de la tarea va a estar basado en un conjuntos de tests. El primero de éstos se basa en el uso de 64 bytes como largo del bloque (\texttt{block\_length}), 10 rondas, y la función de hash \texttt{SHA256}. En este caso se va a fijar una clave secreta, y se van a realizar los siguientes tests:
\begin{itemize}
\item{[0.15 puntos]} Con un mensaje cuyo largo es divisible por \texttt{block\_length} se debe obtener el texto cifrado correcto al utilizar \texttt{enc\_block}.

\item{[0.15 puntos]}  Con un mensaje cuyo largo no es divisible por \texttt{block\_length} se debe obtener una excepción al utilizar \texttt{enc\_block}.

\item{[0.15 puntos]}  Con un texto cifrado cuyo largo es divisible por \texttt{block\_length} se debe obtener el mensaje descifrado correcto al utilizar \texttt{dec\_block}.

\item{[0.15 puntos]}  Con un texto cifrado cuyo largo no es divisible por \texttt{block\_length} se debe obtener una excepción al utilizar \texttt{dec\_block}.

\item{[0.15 puntos]} Con un número de rondas dado se debe obtener la clave correcta al utilizar \texttt{key\_schedule}.

\item{[0.15 puntos]} Con un mensaje cuyo largo no es divisible por \texttt{block\_length} se debe obtener el padding correcto al utilizar \texttt{pad}.

\item{[0.15 puntos]} Con un mensaje cuyo largo es divisible por \texttt{block\_length} se debe obtener el padding correcto al utilizar \texttt{pad}.

\item{[0.15 puntos]} Con un padding correctamente generado utilizando \texttt{pad}, se debe obtener el mensaje original al utilizar \texttt{unpad}.

\item{[0.15 puntos]} Con un padding cuyo largo no es divisible por \texttt{block\_length} se debe obtener una excepción al utilizar \texttt{unpad}.

\item{[0.15 puntos]} Con un padding cuyo largo es divisible por \texttt{block\_length} pero que no sigue las reglas de padding definidas en la tarea, se debe obtener una excepción al utilizar \texttt{unpad}.

\item{[0.15 puntos]} Con un mensaje cuyo largo es divisible por \texttt{block\_length} y en modo de operación \texttt{ECB}, se debe obtener el cifrado correcto al utilizar \texttt{enc}.

\item{[0.15 puntos]} Con un mensaje cuyo largo no es divisible por \texttt{block\_length} y en modo de operación \texttt{ECB}, se debe obtener el cifrado correcto al utilizar \texttt{enc}.

\item{[0.15 puntos]} Con un texto cifrado cuyo largo es divisible por \texttt{block\_length} y en modo de operación \texttt{ECB}, se debe obtener el descifrado correcto al utilizar \texttt{dec}.

\item{[0.15 puntos]} Con un texto cifrado cuyo largo no es divisible por \texttt{block\_length} y en modo de operación \texttt{ECB}, se debe obtener una excepción al utilizar \texttt{dec}.

\item{[0.45 puntos]} Con un mensaje cuyo largo es divisible por \texttt{block\_length} y en modo de operación \texttt{CBC}, se debe obtener un texto cifrado del largo correcto y un vector de inicialización \texttt{IV} al utilizar \texttt{enc}. Al utilizar \texttt{dec} con el texto cifrado y el vector de inicialización \texttt{IV} se debe obtener el mensaje original.

\item{[0.45 puntos]} Con un mensaje cuyo largo no es divisible por \texttt{block\_length} y en modo de operación \texttt{CBC}, se debe obtener un texto cifrado del largo correcto y un vector de inicialización \texttt{IV} al utilizar \texttt{enc}. Al utilizar \texttt{dec} con el texto cifrado y el vector de inicialización \texttt{IV} se debe obtener el mensaje original.
\end{itemize}
Esta parte tiene un total de 3 puntos. Además se va a realizar el mismo conjunto de tests pero considerando un largo de 128 bytes, 15 rondas, y la función de hash \texttt{SHA512}. Este segundo conjunto de tests también tiene 3 puntos en total, los cuales son distribuidos exactamente como en los tests anteriores. Las excepciones mencionadas en los tests pueden ser de cualquier tipo.
